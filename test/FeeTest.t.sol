...
contract FeeTest is Test {
    ...
    function setUp() public {
        ...
        // 4. 获取并验证交易对信息
        try factory.getPair(tokenAddress, baseToken) returns (address pairAddress) {
            // 获取交易对地址
    ...
    function testMevAttack() public {
        ...
        uint256[] memory amounts = router.getAmountsOut(inputAmount, getPath(address(baseToken), address(token)));
        ...
        router.swapExactTokensForTokens(
            mevInputAmount, 0, getPath(address(baseToken), address(token)), mevBot, block.timestamp + blockTime
        );
        vm.stopPrank();

        // 2. Check price impact
        amounts = router.getAmountsOut(inputAmount, getPath(address(baseToken), address(token)));
        uint256 newExpectedOutput = amounts[1];

        console.log("\n=== MEV Impact Analysis ===");
        console.log("New Expected Output:", newExpectedOutput);
        console.log("Price Impact: %s%%", ((expectedOutput - newExpectedOutput) * 100) / expectedOutput);
        ...
        router.swapExactTokensForTokens(
            inputAmount, 0, getPath(address(baseToken), address(token)), address(this), block.timestamp + blockTime
        );

        uint256 actualReceived = IERC20(address(token)).balanceOf(address(this)) - initialBalance;

        console.log("Actually Received:", actualReceived);
        console.log("MEV Loss: %s%%", ((newExpectedOutput - actualReceived) * 100) / newExpectedOutput);
    ...
    function formatDecimal(int256 value, uint8 decimals) internal pure returns (string memory) {
        ...
        return string.concat(isNegative ? "-" : "", uint2str(wholePart), ".", decimalStr);
    }
